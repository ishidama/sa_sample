...
def evaluate_energy(state, sizes):
    energy = 0
...
def neighbor(state, sizes):
    new_state = state[:]
    new_sizes = sizes[:]
    i, j = random.sample(range(len(state)), 2)
    new_state[i], new_state[j] = new_state[j], new_state[i]
    new_sizes[i], new_sizes[j] = new_sizes[j], new_sizes[i]
    return new_state, new_sizes


# 初期状態をランダムに生成
state = [random.choice(colors) for _ in range(num_circles)]
sizes = [random.randint(10, 100) for _ in range(num_circles)]
current_energy = evaluate_energy(state, sizes)
best_state = state[:]
best_sizes = sizes[:]
...
for i in range(num_circles):
    circle = plt.Circle(
        (i * 2, 0), sizes[i]/50, edgecolor='black', facecolor=state[i])
...
def update(frame):
    global state, sizes, current_energy, best_state, best_sizes, best_energy

    t = initial_temp / (1 + cooling_rate * frame)
    new_state, new_sizes = neighbor(state, sizes)
    new_energy = evaluate_energy(new_state, new_sizes)
